...
function resizeVideoMedia() {
  ...
  let max = 0;
  let optional =
    isHideMeActive && videoMediaContainer.childElementCount <= 2 ? 1 : 0;
  ...
  const avatarSize = isOneVideoElement
    ? Math.min(200, Math.max(120, Height * 0.25))
    : max / 3;
...
function setWidth(
  ...
  for (let s = 0; s < Tiles.length; s++) {
    ...
    if (isOneVideoElement) {
      ...
      let camHeigh = Tiles[s].style.height.substring(
        0,
        Tiles[s].style.height.length - 2,
      );
