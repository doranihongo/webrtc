  ...
  handleMessage(event) {
    if (event.data.type === "request-wasm") {
      ...
      this.uiManager.updateStatus(
        "✅ RNNoise WASM initialized successfully",
        "success",
      );
    } else if (event.data.type === "wasm-error") {
      this.uiManager.updateStatus(
        "❌ RNNoise WASM error: " + event.data.error,
        "error",
      );
...
class WasmLoader {
  ...
  async loadWasmBuffer() {
    try {
      ...
      this.uiManager.updateStatus(
        "📦 Sending sync module to worklet...",
        "info",
      );
      ...
      this.uiManager.updateStatus(
        "❌ Failed to load sync module: " + error.message,
        "error",
      );
    ...
    try {
      ...
      if (this.audioContext.state === "suspended") {
        try {
          ...
          this.uiManager.updateStatus(
            "🎵 AudioContext resumed after suspend",
            "info",
          );
        } catch (e) {
          this.uiManager.updateStatus(
            "⚠️ AudioContext could not be resumed: " + e.message,
            "warning",
          );
        }
      }
      ...
      await this.audioContext.audioWorklet.addModule(
        "../js/noiseSuppressionProcessor.js",
      );
