...
async function startServer() {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('join-room', ({ roomId, userName, clientId }: { roomId: string; userName: string; clientId: string }) => {
      ...
      // Check if user is already in the room (e.g. page refresh ghost connection)
      if (clientId) {
        for (const [existingSocketId, existingUser] of room.entries()) {
          if (existingUser.clientId === clientId) {
            // Remove old ghost connection
            room.delete(existingSocketId);
            // Notify others that the old connection left so they don't see duplicates
            socket.to(cleanRoomId).emit('user-left', {
              socketId: existingSocketId,
              userName: existingUser.userName,
            });
            console.log(`[Socket] Removed ghost connection ${existingSocketId} for clientId ${clientId}`);
            // Note: We don't forcefully disconnect the old socket here as it might be dead anyway,
            // but removing it from the room map prevents duplicate users and room-full errors.
          }
        }
      }
    ...
    // Update User Name
    socket.on('update-user-name', ({ roomId, userName }: { roomId: string; userName: string }) => {
      const room = rooms.get(roomId);
      if (room && room.has(socket.id)) {
        const user = room.get(socket.id)!;
