...
// Handle bad requests (e.g. invalid JSON body)
    ...
    if (Object.keys(req.query).length > 0) {
        ...
        */
        ...
        if (room && (hostCfg.authenticated || isPeerValid)) {
            return res.sendFile(path.join(dir.public, 'index.html'));
        } else {
...
app.get('/join/:roomId', function (req, res) {
    ...
    if (allowRoomAccess) {
        ...
        res.sendFile(path.join(dir.public, 'index.html')); // Mở index.html để React vẽ màn hình Waiting
    } else {
        res.redirect('/');
    }
});
...
app.get('/brand', (req, res) => {
    res.status(200).json({ message: config.brand && config.brand.htmlInjection ? config.brand : false });
});
...
// Get Signature to compare it later
    ...
    const theirBuf = Buffer.from(String(slackSignature || ''));
    const validSignature =
        sigBuf.length === theirBuf.length && crypto.timingSafeEqual(sigBuf, theirBuf);
...
// Giao quyền kiểm soát Router cho React SPA
app.get('*', (req, res, next) => {
    // Bỏ qua các API, Webhook, socket
    if (req.originalUrl.startsWith('/api/') || 
        req.originalUrl.startsWith('/socket.io/') || 
        req.originalUrl === '/slack') {
        return next();
    }
    res.sendFile(path.join(dir.public, 'index.html'));
});
